...
function fieldHTML(f){let star=f.required?'<span class="req">*</span>':"",i;
if(f.type==="textarea")i=`<textarea name="${esc(f.id)}" rows="4" placeholder="${esc(f.placeholder||"")}" ${f.required?"required":""}></textarea>`;
else if(f.type==="select")i=`<select name="${esc(f.id)}" ${f.required?"required":""}><option value="">Select ${esc(f.label)}</option>${(f.options||[]).map(o=>`<option>${esc(o)}</option>`).join("")}</select>`;
else i=`<input name="${esc(f.id)}" type="${esc(f.type||"text")}" placeholder="${esc(f.placeholder||"")}" ${f.required?"required":""}>`;
...
<section class="step"><div class="section-head"><div class="section-icon">✓</div><div><h2>Review & Submit</h2><p>Check everything before sending.</p></div></div><div id="review" class="review-grid"></div><p class="help">Your information is used only to process your enquiry and contact you.</p></section>
...
document.getElementById("share").onclick=async()=>{try{if(navigator.share)await navigator.share({title:state.brand,text:"Open EZEE VISION CHAMPUA enquiry form",url:location.href});else{await navigator.clipboard.writeText(location.href);alert("Form link copied.")}}catch{}};
show()
}
...
let cloud=false;
if(API){try{const r=await fetch(API,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"create",data:item})});cloud=r.ok}catch{}}
item.syncStatus=cloud?"cloud":"local";save(state);
form.innerHTML=`<div class="success-screen"><div class="success-icon">✓</div><h2>Response Submitted Successfully</h2><p class="help">Thank you for contacting <b>${esc(state.brand)}</b>.</p><p class="help">Response ID: <b>${esc(id)}</b></p>${cloud?'<p class="cloud-status">☁ Saved to secure cloud storage</p>':'<p class="cloud-status off">✓ Saved on this device</p>'}<br><br><button type="button" class="btn primary" onclick="location.reload()">Submit Another Response</button></div>`;
}
render();
